...
<body class="bg-gray-900 font-sans leading-normal tracking-normal text-gray-200" id="body">
    ...
    <div class="container mx-auto mt-8">
        ...
        <!-- Add Task Form -->
        <form action="{{ route('tasks.store') }}" method="POST" class="mb-8 bg-gray-800 p-6 rounded-lg shadow-md">
            @csrf
            <div class="mb-4">
                <label for="title" class="block text-gray-300 font-bold mb-2">Task Title:</label>
                <input type="text" name="title" placeholder="Task Title" class="w-full px-3 py-2 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-600 bg-gray-700 text-gray-100" required>
            </div>
            <div class="mb-4">
                <label for="description" class="block text-gray-300 font-bold mb-2">Task Description:</label>
                <textarea name="description" placeholder="Task Description" class="w-full px-3 py-2 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-600 bg-gray-700 text-gray-100"></textarea>
            </div>
            <button type="submit" class="bg-blue-700 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition duration-300">Add Task</button>
        </form>
            ...
            @foreach($tasks as $task)
                <li class="bg-gray-800 p-4 rounded-lg shadow flex justify-between items-center hover:bg-gray-700 transition duration-300">
                    ...
                    <div class="flex space-x-2">
                        <a href="{{ route('tasks.edit', $task->id) }}" class="text-blue-400 hover:underline">Edit</a>
